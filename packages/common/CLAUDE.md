...
- `validation` — env handling. `validateEnv(zodShape)` parses `process.env` and **throws** `Env validation failed` on error (used by every service `config.ts` and package config). Prebuilt shapes `NodeValidationSchema` (PORT, NODE_ENV — `development` | `production` | `test`, the last one only so jest specs that transitively import a config module do not die at import time) and `DatabaseValidationSchema` (DATABASE_URL); type helper `SchemaTypeOf<Shape>`; derived `NodeEnvironment` / `DatabaseEnvironment`.
